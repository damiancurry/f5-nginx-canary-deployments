...
echo $oldsvc $newsvc
sed "s|existing-svc|${oldsvc}|g" kic/header-split.yaml | sed "s|new-svc|${newsvc}|g" | sed "s|elb-hostname|${ichostname}" > header-split.yaml
...
sleep 30
echo $IC_IP
headerreturncode=`curl -i -s -o /dev/null -w "%{http_code}" -H "release: beta" https://$ichostname/`
...
then
    ...
    sed "s|new-svc|${oldsvc}|g" kic/single-svc-virtualserver.yaml | sed "s|elb-hostname|${ichostname}"  > single-svc-virtualserver.yaml
...
sed "s|existing-svc|${oldsvc}|g" kic/weight-split.yaml | sed "s|new-svc|${newsvc}|g" | sed "s|new-weight|${newweight}|g" | sed "s|old-weight|${oldweight}|g" | sed "s|elb-hostname|${ichostname}" > weight-split.yaml
...
while [[ $c -le 50 ]]
do 
   curl -i -s -o /dev/null https://$ichostname/foo
   #curl -i -s -o /dev/null https://$ichostname/
   let c=c+1
done
...
sed "s|new-svc|${newsvc}|g" kic/single-svc-virtualserver.yaml | sed "s|elb-hostname|${ichostname}"  > single-svc-virtualserver.yaml
kubectl apply -f single-svc-virtualserver.yaml --namespace $namespace
exit 0
